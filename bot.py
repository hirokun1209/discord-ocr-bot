...
# ===== 分割関数（上1/6＋下4分割） =====
def split_preview_top1_bottom4(center_raw):
    w, h = center_raw.size
    parts = []
...
async def on_message(message):
    ...
    if message.content.strip() == "!test":
        await message.channel.send("✅ BOT動いてるよ！（上1/6＋下4分割プレビュー版）")
        return

    if message.attachments:
        await message.channel.send("📥 画像を受け取りました、中央OCR領域を上1/6＋下4分割して確認します…")
        ...
        for attachment in message.attachments:
            ...
            # 分割プレビュー（上1/6＋下4分割）
            parts = split_preview_top1_bottom4(center_raw)
